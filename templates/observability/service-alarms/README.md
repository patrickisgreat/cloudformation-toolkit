...
|-----------|-------------|------|---------|
| `NamePrefix` | Prefix for alarm names. | `String` | **required** |
| `Environment` | Environment being monitored.<br>Allowed: `dev`, `staging`, `prod` | `String` | `dev` |
| `ServiceSuffix` | Service these alarms belong to. | `String` | `app` |
| `AlarmTopicArn` | SNS topic every alarm notifies, from messaging/sns-topic. Verify the subscription is confirmed - an unconfirmed email subscription makes the whole alarm set silent while looking healthy. | `String` | **required** |
| `LoadBalancerFullName` | Load balancer full name (app/<name>/<id>) from containers/alb. Leave empty to skip the load balancer alarms. | `String` | `""` |
| `TargetGroupFullName` | Target group full name from containers/fargate-service. Required with the load balancer. | `String` | `""` |
| `EcsClusterName` | ECS cluster name. Leave empty to skip the saturation alarms. | `String` | `""` |
| `EcsServiceName` | ECS service name. Required with the cluster name. | `String` | `""` |
| `DeadLetterQueueName` | Dead-letter queue name from messaging/sqs-queue. A non-zero DLQ is the single most useful signal a queue-backed system produces. Leave empty to skip. | `String` | `""` |
| `Http5xxThreshold` | 5xx responses in a minute before alarming. Counts responses generated by the targets, not by the load balancer - a target that is down produces 5xx from the ALB itself and shows up in the target-health alarm instead. | `Number` | `10` |
| `LatencyThresholdSeconds` | p99 target response time, in seconds, sustained before alarming. p99 rather than average: an average hides the tail, and the tail is what users notice. | `Number` | `2` |
| `CpuThresholdPercent` | Service CPU utilisation before alarming. Set it above the autoscaling target - alarming at the level the scaler is aiming to hold pages you every time scaling works correctly. | `Number` | `85` |
| `MemoryThresholdPercent` | Service memory utilisation before alarming. | `Number` | `85` |
| `DeadLetterMessageThreshold` | Messages in the dead-letter queue before alarming. | `Number` | `1` |

### Outputs

| Output | Description | Exported as |
|--------|-------------|-------------|
| `AlarmNames` | Comma-separated names of the alarms created, for a composite alarm or a dashboard. | — |
| `AlarmTopicArn` | Topic the alarms notify, echoed for runbooks. | — |

### Resources

| Logical ID | Type | Created when |
|------------|------|--------------|
| `Http5xxAlarm` | `AWS::CloudWatch::Alarm` | `WatchesLoadBalancer` |
| `UnhealthyTargetsAlarm` | `AWS::CloudWatch::Alarm` | `WatchesLoadBalancer` |
| `LatencyAlarm` | `AWS::CloudWatch::Alarm` | `WatchesLoadBalancer` |
| `CpuAlarm` | `AWS::CloudWatch::Alarm` | `WatchesEcsService` |
| `MemoryAlarm` | `AWS::CloudWatch::Alarm` | `WatchesEcsService` |
| `DeadLetterQueueAlarm` | `AWS::CloudWatch::Alarm` | `WatchesDeadLetterQueue` |
<!-- END_CFN_DOCS -->
